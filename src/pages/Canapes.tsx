...
import FormuleStep from "../components/Canapé/CanapeTypeStep";
import OptionsStep from "../components/Canapé/CanapeOptionsStep";
import ContactStep from "../components/Canapé/Contact";
import VerticalProgressBar from "../components/Canapé/VerticalProgressBar";
...
export default function Canapes() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const contactRef = sectionRefs.current[2];
      const contactTop = contactRef?.getBoundingClientRect().top || 0;

      if (contactTop < window.innerHeight && (!selectedFormule || selectedOptions.length === 0)) {
        window.scrollTo({ top: (sectionRefs.current[1]?.offsetTop || 0), behavior: 'smooth' });
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [selectedFormule, selectedOptions]);
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-white to-gray-50">
        ...
        <div
          ...
          style={{ backgroundImage: "url('/2d/tout/canape1.jpg')" }}
        >
